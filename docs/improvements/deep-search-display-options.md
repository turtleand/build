---
title: Deep Search display concepts
description: Three UI patterns for surfacing extended Deep Search content from a blog post.
lastUpdated: 2025-??-??
---

# Goal

When a reader clicks the “Deep Search” (a.k.a. DeepSeek) link on a post, surface the long-form content without forcing them to leave the current context. Below are three patterns tuned for the blog’s current Astro stack and component library.

## 1. Full-screen modal overlay

* **Trigger:** Use the existing “Deep Search companion post” link, but intercept it with a client-side click handler.
...
* **Implementation notes:**
  * Lazy-load the Deep Search markdown via an API route/edge function and render inside the modal with the same Markdown component the blog already uses.
...
* **Pros:** Keeps the reader anchored to the original article; reinforces that the Deep Search is a supplemental layer.
...
* **Trigger:** Clicking the Deep Search CTA opens a side drawer that slides from the right (desktop) or bottom (mobile).
* **Behavior:** Drawer covers ~40% width on desktop, 90% height on mobile, and displays the Deep Search article with its own scroll.
* **Implementation notes:**
...
* **Trigger:** Clicking the Deep Search link smoothly scrolls to an accordion-like section injected directly below the CTA.
* **Behavior:** The section expands in-place, renders the Deep Search markdown, and includes “Collapse” / “Open in new tab” controls.
* **Implementation notes:**
  * Preload the Deep Search content during build (both posts share a `translationKey`), so expanding only toggles visibility and avoids client fetches.
...
**Recommendation:** Prototype the inline expansion first (concept 3) because it fits the content-heavy nature of Deep Search articles and keeps SSR. Use the modal or drawer when you need a stronger separation or want analytics on “engaged research mode” sessions. Each approach should share a single Deep Search data source so content stays in sync regardless of presentation.

## Current failure (why the inline expansion is still broken)

* The Deep Search link in the main post continues to navigate to the companion URL and returns 404 instead of expanding inline. That means the client-side interception isn’t firing for that anchor (likely because the rendered href doesn’t exactly match the string we’re watching, or the DOM is different when Markdown renders the link).
* The companion page exists (`2025-11-29-mocker-python-deep-search.md` with `slug: "2025-11-29-mocker-python-deep-search"`), so the 404 is a routing/refresh issue rather than missing content. The inline accordion depends on the pre-rendered deep-search entry; navigation shouldn’t happen at all.
